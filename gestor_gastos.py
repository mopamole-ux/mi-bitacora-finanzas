...
# --- NUEVO: CONTROL DE SALDO BASE ---
with st.sidebar:
    st.header("Configuración")
    disponible_banco = st.number_input("💰 Saldo Base (Banco)", value=20000.0, step=100.0, format="%.2f")
    st.info("Este saldo se usa como punto de partida para la gráfica y el disponible estimado.")
...
with tab_analisis:
    if not df_man.dropna(subset=['Monto', 'Fecha']).empty:
        ...
        mc1.metric("Saldo Base", f"${disponible_banco:,.2f}")
        mc2.metric("Total Gastos", f"${total_g:,.2f}", delta_color="inverse")
        ...
        fig_line = px.area(diario, x='Fecha_DT', y='Saldo_Proyectado', 
                          line_shape="hv", markers=True, title="Trayectoria del Saldo")
        ...
        with gc1:
            fig_pie = px.pie(df_p[df_p['Tipo'] == 'Gasto'], values='Monto', names='Categoria', title="Gastos por Categoría")
            st.plotly_chart(fig_pie, use_container_width=True)
        with gc2:
            fig_bar = px.bar(df_p.groupby('Tipo')['Monto'].sum().reset_index(), x='Tipo', y='Monto', color='Tipo', title="Comparativa Abono vs Gasto")
